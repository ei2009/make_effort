...
struct vigor{
    ...
    int coin_toss_power=15; //コイントスのデフォルトのダメージ
    ...
    vector<int> skill; //抽選された技の番号（0インデックス)
    ...
    void skill_display(int i){ //技の表示
        cout<<"行動を選択"<<endl;
        for(int j=1;j<=number_of_use_skills[i];j++){
            cout<<j<<" : "<<skill_name[skill[j-1]]<<endl;
        }
        return;
    }
    ...
    void turn_start(int i){ //ターン開始時の処理
        ...
        if(player_form[i]&(1<<sleep_number)){
            ...
            while(skill.size()<number_of_use_skills[i]){
                ...
                bool flag_enter_skill=true;
                for(int i=0;i<skill.size();i++){
                    if(skill[i]==skill_number){
                        flag_enter_skill=false;
                    }
                }
                if(flag_enter_skill){
                    skill.push_back(skill_number);
                }
            }
            skill_display(i);
            int command;
